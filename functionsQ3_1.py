...
def genDataRows(stock, maxPreds, maxPreds3):
    row = [str(stock), str(maxPreds), str(maxPreds3)]
    return row
...
def updateGoLstate(side, currState, predators, repoStock, stockGain, stateType):
        ...
        for j in range(len(currState[i])):
            ...
            if (currState[i][j] != 2):
                ...
                if currState[i][j] == 1 and not (neighbors == 2 or neighbors == 3):
                    newState[i][j] = 0
                #Rule 4:
                #   If alive, and 4 neighbors, live
                if stateType == 3:
                    if currState[i][j] == 1 and neighbors == 4:
                        newState[i][j] = 1
            else:
                newState[i][j] = 0
       
    
    newState = updatePred(side, newState, predators, repoStock, stockGain)
    del currState
    return newState
